...
def _generaGetParam(dictEntrada):
    paramdict = {}
    for k in dictEntrada.keys():
        v = dictEntrada[k]
        paramdict[k] = v
...
def _getsearchParam(dictEntrada, serializer, template=None, prefix="s_"):
    ...
    for k in dictEntrada.keys():
        ...
        if k.startswith("f_"):
            ...
            try:
                ...
                for f in serverFilters:
                    if 'tipo' in f and f['tipo'] == "q":
                        qdict.append((f["criterio"], f["valor"]))
                    else:
        ...
        # if fqdict:
        #     print("tengo or")
        #     fqqueries = [Q(x) for x in fqdict]
        #     if fquery is None:
        #         fquery = fqqueries.pop()
        #     for item in fqqueries:
        #         fquery |= item
        # if fsdict:
        #     print("pero not engo and")
        #     fsqueries = [Q(x) for x in fsdict]
        #     if fquery is None:
        #         fquery = fsqueries.pop()
        #     for item in fsqueries:
        #         fquery &= item
...
def _getGridFilters(prefix, usuario):
    ...
    if sis_gridfilter:
        try:
            ...
            pass
        try:
            alluser_gridfilter = sis_gridfilter.objects.filter(Q(usuario__isnull=True) & Q(prefix=prefix))
            for obj in alluser_gridfilter:
                fgrid[obj.descripcion] = {}
                fgrid[obj.descripcion]["pk"] = obj.id
                fgrid[obj.descripcion]["filtro"] = obj.filtro
                fgrid[obj.descripcion]["default"] = obj.inicial
