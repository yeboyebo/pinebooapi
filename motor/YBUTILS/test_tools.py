...
class YBWebDriver(ChromeWebDriver):
    ...
    def find_by_name(self, name, avoid_sleep=False):
        resp = super().find_element_by_name(name)

        if self._sleep and not avoid_sleep:
            time.sleep(self._sleep)

        return resp

    def find_by_css(self, css, avoid_sleep=False):
    ...
    def click_by_name(self, name):
        resp = self.find_by_name(name, avoid_sleep=True)
        if not resp:
            return resp
    ...
    def click_by_css(self, css):
        resp = self.find_by_css(css, avoid_sleep=True)
        if not resp:
            return resp
    ...
    def send_keys_by_css(self, css, keys):
        resp = self.find_by_css(css, avoid_sleep=True)
        if not resp:
            return resp
